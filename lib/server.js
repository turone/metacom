...
const createServer = (protocol) =>
  protocol === 'http2'
    ...
    : require(`node:${protocol}`).createServer;
...
class Server {
  ...
  init() {
    ...
    const createServ =
      protocol === 'http' || balancer
        ? createServer('http')
        : createServer(protocol);
    this.apiUriS = apiUri.endsWith('/') ? apiUri : apiUri + '/';
    const opt = { key, cert, noDelay: !nagle, SNICallback };
    if (protocol === 'http2') opt.allowHTTP1 = true;
    this.httpServer = createServ(opt);
    const withOptStatic = () => {
      const checkStaticPath = application.static.initCheckPath(optStatic);
      return (url, transport) => {
        if (checkStaticPath(url))
          return void application.static.serve(url, transport);
        return void this.rest(transport);
      };
    };
    const checkStaticPath = optStatic
      ? withOptStatic()
      : (url, transport) => application.static.serve(url, transport);
    this.httpServer.on('request', async (req, res) => {
      const transport = new HttpTransport(this, req, res);
      const api = req.url.startsWith(apiUri);
      if (!api && !(balancer && req.url === '/')) {
        if (application.static.constructor.name !== 'Static') return;
        return void checkStaticPath(req.url, transport);
      }
  ...
  async rpc(client, packet) {
    ...
    this.console.log(unit, methodName);
  ...
  request(client, transport, data) {
    ...
    const pathname = url.slice(this.apiUriS.length);
  ...
  async rest(transport) {
    const client = new Client(transport);
    const { headers, url, method: verb } = transport.req;
    const [path, params] = metautil.split(url, '?');
    const getPar = params === '' ? false : metautil.parseParams(params);
    const body = await metautil
      .receiveBody(transport.req)
      .then(metautil.jsonParse)
      .catch(() => null);
    const par = { path, getPar, verb, body, headers };
    try {
      const result = await this.application.rest.tree.router(client, par);
      const { data, httpCode, ext = 'html', redirect } = result;
      if (!redirect) transport.write(data, httpCode, ext);
      if (redirect) transport.redirect(data, httpCode);
      //transport.error(501);
    } catch (error) {
      client.error(500, { id: 0, error });
    }
  }
  ...
  balancing(transport) {
    ...
    transport.redirect(
      `${protocol === 'http' ? 'http' : 'https'}://${host}:${targetPort}/`,
    );
  }
